...
if st.session_state.data_fetched:
    ...
    st.subheader("📉 MSE Comparison of Models (80:20 Split —  Without Standardscaler Scaling)")
    choice4 = st.selectbox("select 1 for viewing", options=[0, 1], key='choice4')
    ...
    if choice4 == 1:
            ...
            # Helper function to calculate and store MSE
            def add_mse(model_name, y_true, y_pred):
                mse = mean_squared_error(y_true, y_pred)
                mse_results.append({"Model": model_name, "MSE": round(mse, 4)})


            # --- Common Split ---
            total_len = len(data)
            split_index = int(0.8 * total_len)
            train_data = data['Close'].iloc[:split_index]
            test_data = data['Close'].iloc[split_index:]
            ...
            try:
                ...
                forecast_arima.index = test_data.index  # Align index
                add_mse("ARIMA", test_data, forecast_arima)
            except Exception as e:
                mse_results.append({"Model": "ARIMA", "MSE": f"Error: {str(e)}"})

            # ----- SARIMAX -----
            try:
                sarimax_model = SARIMAX(train_data, order=(1, 1, 1), seasonal_order=(1, 1, 1, 12))
                sarimax_fit = sarimax_model.fit(disp=False)
                forecast_sarimax = sarimax_fit.forecast(steps=len(test_data))
                forecast_sarimax.index = test_data.index
                add_mse("SARIMAX", test_data, forecast_sarimax)
            except Exception as e:
                mse_results.append({"Model": "SARIMAX", "MSE": f"Error: {str(e)}"})
            ...
            try:
                ...
                add_mse("Holt-Winters", test_data, forecast_hw)
            except Exception as e:
                mse_results.append({"Model": "Holt-Winters", "MSE": f"Error: {str(e)}"})
            ...
            try:
                ...
                m = Prophet()
                m.fit(prophet_train)
                future = m.make_future_dataframe(periods=len(prophet_test), freq='B')
                forecast = m.predict(future)
                pred_prophet = forecast[['ds', 'yhat']].set_index('ds').loc[prophet_test['ds']]
                add_mse("Prophet", prophet_test['y'].values, pred_prophet['yhat'].values)
            except Exception as e:
                mse_results.append({"Model": "Prophet", "MSE": f"Error: {str(e)}"})
            ...
            try:
                ...
                X = df_feat[['lag1', 'lag2', 'lag3', 'dayofweek', 'month']]
                y = df_feat['Close']
                split_feat = int(0.8 * len(X))
                X_train, X_test = X.iloc[:split_feat], X.iloc[split_feat:]
                y_train, y_test = y.iloc[:split_feat], y.iloc[split_feat:]

                rf = RandomForestRegressor()
                rf.fit(X_train, y_train)
                y_pred_rf = rf.predict(X_test)
                add_mse("Random Forest", y_test, y_pred_rf)
            except Exception as e:
                mse_results.append({"Model": "Random Forest", "MSE": f"Error: {str(e)}"})
            ...
            try:
                ...
                add_mse("HistGradientBoosting", y_test, y_pred_hgb)
            except Exception as e:
                mse_results.append({"Model": "HistGradientBoosting", "MSE": f"Error: {str(e)}"})
            ...
            try:
                ...
                add_mse("SVR", y_test, y_pred_svr)
            except Exception as e:
                mse_results.append({"Model": "SVR", "MSE": f"Error: {str(e)}"})
            ...
            try:
                ...
                for i in range(sequence_length, len(scaled)):
                    X_lstm.append(scaled[i - sequence_length:i, 0])
                ...
                y_test_inv = scaler_lstm.inverse_transform(y_test_lstm.reshape(-1, 1)).flatten()
                y_pred_inv = scaler_lstm.inverse_transform(y_pred_lstm).flatten()
                add_mse("LSTM", y_test_inv, y_pred_inv)
            except Exception as e:
                mse_results.append({"Model": "LSTM", "MSE": f"Error: {str(e)}"})

            # Final Table
            st.markdown("### ✅ Final MSE Comparison Table (Unscaled)")
            st.dataframe(pd.DataFrame(mse_results))
    else:
            st.write("click 1 to view it")
