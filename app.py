...
st.subheader("📉 MSE Comparison of Models (80:20 Split — With & Without Scaling)")

if st.button("Run MSE Comparisons (Original vs Scaled)"):
    mse_results = []
    ...
    ### Helper for adding MSE result
    def add_result(name, orig, scaled):
        mse_results.append({"Model": name, "MSE (Original)": orig, "MSE (Scaled)": scaled})

    ### Random Forest
    rf = RandomForestRegressor()
    rf.fit(X_train, y_train)
    y_pred_rf_orig = rf.predict(X_test)
    rf.fit(X_train, y_train_scaled)
    y_pred_rf_scaled = rf.predict(X_test)
    y_pred_rf_scaled_inv = y_scaler.inverse_transform(y_pred_rf_scaled.reshape(-1, 1)).flatten()
    add_result("Random Forest", mean_squared_error(y_test, y_pred_rf_orig), mean_squared_error(y_test, y_pred_rf_scaled_inv))

    ### HistGradientBoosting
    hgb = HistGradientBoostingRegressor()
    hgb.fit(X_train, y_train)
    y_pred_hgb_orig = hgb.predict(X_test)
    hgb.fit(X_train, y_train_scaled)
    y_pred_hgb_scaled = hgb.predict(X_test)
    y_pred_hgb_scaled_inv = y_scaler.inverse_transform(y_pred_hgb_scaled.reshape(-1, 1)).flatten()
    add_result("HistGradientBoosting", mean_squared_error(y_test, y_pred_hgb_orig), mean_squared_error(y_test, y_pred_hgb_scaled_inv))
    ...
    y_pred_svr_orig = svr.predict(X_test_scaled)

    svr.fit(X_train_scaled, y_train_scaled)
    y_pred_svr_scaled = svr.predict(X_test_scaled)
    y_pred_svr_scaled_inv = y_scaler.inverse_transform(y_pred_svr_scaled.reshape(-1, 1)).flatten()
    add_result("SVR", mean_squared_error(y_test, y_pred_svr_orig), mean_squared_error(y_test, y_pred_svr_scaled_inv))

    ### ARIMA
    try:
        arima = ARIMA(data['Close'], order=(5, 1, 0))
        arima_fit = arima.fit()
        forecast_arima = arima_fit.predict(start=split_index, end=len(data) - 1, typ='levels')
        add_result("ARIMA", mean_squared_error(y_test, forecast_arima), "-")
    except:
        add_result("ARIMA", "Failed", "-")

    ### SARIMAX
    try:
        sarimax = SARIMAX(data['Close'], order=(1, 1, 1), seasonal_order=(1, 1, 1, 12))
        sarimax_fit = sarimax.fit(disp=False)
        forecast_sarimax = sarimax_fit.predict(start=split_index, end=len(data) - 1)
        add_result("SARIMAX", mean_squared_error(y_test, forecast_sarimax), "-")
    except:
        add_result("SARIMAX", "Failed", "-")

    ### Holt-Winters
    try:
        hw = ExponentialSmoothing(data['Close'], trend='add', seasonal='add', seasonal_periods=12)
        hw_fit = hw.fit()
        forecast_hw = hw_fit.forecast(len(y_test))
        add_result("Holt-Winters", mean_squared_error(y_test, forecast_hw), "-")
    except:
        add_result("Holt-Winters", "Failed", "-")
    ...
    try:
        ...
        future = model_p.make_future_dataframe(periods=len(test_prophet), freq='B')
        forecast = model_p.predict(future)
        pred_prophet = forecast.set_index('ds').loc[test_prophet['ds'], 'yhat']
        add_result("Prophet", mean_squared_error(test_prophet['y'], pred_prophet), "-")
    except:
        add_result("Prophet", "Failed", "-")

    ### LSTM
    try:
        df_lstm = data[['Close']].copy()
        scaler_lstm = StandardScaler()
        scaled_lstm = scaler_lstm.fit_transform(df_lstm)

        sequence_length = 60
        X_lstm, y_lstm = [], []
        for i in range(sequence_length, len(scaled_lstm)):
            X_lstm.append(scaled_lstm[i - sequence_length:i, 0])
            y_lstm.append(scaled_lstm[i, 0])
        ...
        add_result("LSTM", "-", mean_squared_error(y_test_lstm_inv, y_pred_lstm_inv))
    except:
        add_result("LSTM", "-", "Failed")

    # Show Table
    st.markdown("### 📊 MSE Comparison Table (Original Scale vs Scaled `Close`)")
    st.dataframe(pd.DataFrame(mse_results))
        st.markdown("""
            ## 🙏 Thank You!
